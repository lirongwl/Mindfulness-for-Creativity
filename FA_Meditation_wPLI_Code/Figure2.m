%% Plot_Network_Correlations_ROI_RAT2-RAT1 Alpha波 + FC1-{F7, FC5} 
...
% =========================================================================
% 更新为您指定的数据文件名
dataFile = 'Individual_wPLI_Data_RAT1RAT2_Alpha.mat'; 
...
% =========================================================================
stats_CT = struct('mean_diff', zeros(1,num_targets), 'r_corr', zeros(1,num_targets), 'p_corr', zeros(1,num_targets));
stats_FA = struct('mean_diff', zeros(1,num_targets), 'r_corr', zeros(1,num_targets), 'p_corr', zeros(1,num_targets));

% 用于存储组间统计结果的数组
p_diff_wPLI_edges = zeros(1,num_targets);
p_diff_corr_edges = zeros(1,num_targets);
z_diff_corr_edges = zeros(1,num_targets);
...
% 3. 导出统计结果到 Excel
% =========================================================================
Connection_Names = {'FC1-F7'; 'FC1-FC5'; 'FC1-Target_ROI'};
...
% 写入 Excel (文件名带上口径后缀)
excel_filename = sprintf('Between_Group_Statistics_FC1_Network_%s.xlsx', OUTLIER_MODE);
...
color_scatter = [0.35, 0.55, 0.75]; % 灰蓝色

% 加宽画布，为左侧的全局行标签留出空间
fig_name = sprintf('Network & Correlation FC1 (%s)', OUTLIER_MODE);
...
% --- 提取 FC1-F7 的单一连接数据用于散点图 (F7 是 target_nodes 的第 1 个元素) ---
% 必须使用 remove_outliers 后的 clean 数据来画散点图
data_FA_F7_raw = squeeze(wPLI_DIFF_FA(:, 1, 1));
[data_FA_F7_clean, FA_DIFF_F7_clean, ~] = remove_outliers(data_FA_F7_raw, FA_DIFF, global_z_thresh);

data_CT_F7_raw = squeeze(wPLI_DIFF_CT(:, 1, 1));
[data_CT_F7_clean, CT_DIFF_F7_clean, ~] = remove_outliers(data_CT_F7_raw, CT_DIFF, global_z_thresh);

% --- Row 1: FA Group ---
draw_heatmap(pos_A_heat, mean_matrix_FA, chan_labels, '', heatmap_clims);
draw_network(pos_B_brain, stats_FA, '', color_inc, color_dec, color_nonsig, global_max_diff, num_targets);
% 传入 clean 数据
draw_scatter(pos_C_scat, data_FA_F7_clean, FA_DIFF_F7_clean, stats_FA.r_corr(1), stats_FA.p_corr(1), 'F7', '', color_scatter, x_limits, y_limits);

% --- Row 2: CT Group (标题留空) ---
draw_heatmap(pos_D_heat, mean_matrix_CT, chan_labels, '', heatmap_clims);
draw_network(pos_E_brain, stats_CT, '', color_inc, color_dec, color_nonsig, global_max_diff, num_targets);
% 传入 clean 数据
draw_scatter(pos_F_scat, data_CT_F7_clean, CT_DIFF_F7_clean, stats_CT.r_corr(1), stats_CT.p_corr(1), 'F7', '', color_scatter, x_limits, y_limits);
...
% 在两个散点图之间添加组间差异显著性的标注
p_diff_F7 = p_diff_corr_edges(1); % 获取 FC1-F7 的组间差异 P 值
text(0.825, 0.50, sprintf('Interaction: {\\it P} = %.3f', p_diff_F7), ...
    'FontSize', 14, 'FontName', 'Arial', 'FontWeight', 'bold', 'Color', [0.8 0.1 0.1], ...
...
% 保存高分辨率图片 (文件名带上口径后缀)
pdf_name = sprintf('Network_Correlations_FC1_F7_%s.pdf', OUTLIER_MODE);
png_name = sprintf('Network_Correlations_FC1_F7_%s.png', OUTLIER_MODE);
exportgraphics(fig, pdf_name, 'ContentType', 'vector');
exportgraphics(fig, png_name, 'Resolution', 600);
fprintf('🎉 顶刊排版级高质量图片已保存: %s\n', png_name);
...
function draw_network(ax_pos, stats, title_str, color_inc, color_dec, color_nonsig, global_max_diff, num_targets)
    axes('Position', ax_pos); hold on;
    % 更新电极坐标：FC1(左侧额中), F7(极左额叶), FC5(左侧额中偏外)
    nodes_info = {'FC1', -0.20, 0.20; 
                  'F7',  -0.70, 0.50; 
                  'FC5', -0.60, 0.20};
...
function draw_scatter(ax_pos, x_data, y_data, r_val, p_val, target_name, title_str, color_scatter, x_lims, y_lims)
    ...
    xlabel(sprintf('\\Delta wPLI (FC1 - %s)', target_name), 'FontSize', 15, 'FontWeight', 'bold');
